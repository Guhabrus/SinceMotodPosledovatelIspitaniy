...
void Solve()
{
	...
	SensingElement TZMK(0.05, 1200.0, 144.0);	TZMK.gran = 3000.0; TZMK.ac = 0.0000001;				/* Кф теплопроводности, Удельная теплоёмкость, плотность */

	TZMK.Print_InversHeatTransferProblemGradiend = true;

	TZMK.SetStartHeatFlux(5000.0);	TZMK.SetStartTemp(300.0);
	...
	for (int j = 0; j < sT; j++)
		Tx1[j] = Example1.GetSplineNode(j*TZMK.GetStepTime())* 5.;											/*Вывод точек сплайна в массив граничного условия*/




	int Board = 0;
	TZMK.DirectHeatTransferProblemSecondGY(Tx1);	TZMK.PrintTempInX1Point();						/*Решение прямой задачи - определение поля температур*/
	...
	KKK.SetStartHeatFlux(80000.0);	KKK.SetStartTemp(300.0);
